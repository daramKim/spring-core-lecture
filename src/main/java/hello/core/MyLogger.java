...
* @Scope 의 proxyMode = ScopedProxyMode.TARGET_CLASS) 를 설정하면 스프링 컨테이너는 CGLIB 라는 바이트코드를 조작하는 라이브러리를
* 사용해서 MyLogger를 상속받은 가짜 프록시 객체를 생성한다. 결과를 확인해보면 우리가 등록한 순수한 MyLogger 클래스가 아니라
* "MyLogger$$EnhancerBySpringCGLIB" 라는 클래스로 만들어진 객체가 대신 등록된 것을 확인할 수 있다.
